...
| `molt_reader` vs a per-Auditor login | The reader role is what Molt's own read-only components connect as. An Auditor connects as its own database login, which holds `SELECT` on that Auditor's own view set and nothing else. The reader role is deliberately never granted to an Auditor: it would widen an untrusted external party's reach to everything the reader can see across every Client, where the per-Auditor grant is scoped to that Auditor's own Client |
...
**Auditor** — An untrusted third-party security reviewer acting for a departing Client, granted read-only interrogation access. Explicitly untrusted, which is why the access is read-only, row-filtered to that Auditor's own Client, granted on an explicit view list, and held by that Auditor's own database login, created with a validity bound of at most 30 days and holding `SELECT` on those views directly. One control-plane service account is created per Auditor beside that login. *Compare:* `molt_reader`, which is not granted to an Auditor. Granting it would widen an untrusted external party's reach to everything the reader role can see across every Client; the per-Auditor grant reaches one Client's views and nothing else.
...
**Least-privilege role** — The principle applied twice over: one cloud execution role per component with a named permission set and no wildcard resource, and one database role per component. The database roles are `molt_writer` for the Collector, `molt_eraser` for the erasure path, `molt_reader` for verification, sensitivity analysis, and the MCP server, and `molt_watcher` for the policy path. The Auditor views are the deliberate exception: `SELECT` on them is granted to each Auditor's own login rather than through `molt_reader`, because granting the reader role to an untrusted external party would widen its reach to everything that role can see across every Client, while the per-Auditor grant is scoped to that Auditor's own Client. The eraser role's `DELETE` is revoked on every audit-evidence table, so the role that removes memory content cannot remove the record of having removed it.
...
**Auditor_Gateway** — The read-only access path exposing the cluster to an Auditor through the Managed_MCP_Server. Not a running service: a provisioned access path plus generated connection instructions holding no credential value. The provisioning creates one schema per Auditor holding a Ledger view, an Erasure_Run view, a Client_Binding view, and an as-of-attribution view, each filtered to that Auditor's Client, and grants `SELECT` on those views to the Auditor's own login rather than through `molt_reader`.
...
**CI_Workflow** — The Repository's continuous integration workflow definition, running six steps in a fixed order: strict static type check, type-ignore allowlist check, linter, formatter check, metadata-hygiene check, then a sixth step running three suites — the unit suite, the quality suite, and the property suite, in that order, because the quality suite holds the gates over the suites themselves and the deadline convention it checks is the gate on the property suite. Any failing step fails the workflow. It requires no cloud or cluster credential, which is what makes it runnable by a reviewer.
