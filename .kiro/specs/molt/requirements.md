...
9. THE Memory_Store SHALL refuse an insert of a Session whose spawning Event identifier names no existing Ledger row and SHALL write nothing for that Session, and SHALL raise that refusal from the write path itself inside the inserting transaction rather than rest it on a foreign key over the spawning Event column, because a sub-agent Session names the Event that spawned it while every Event names the Session it was recorded in, so that reference stands between an authorised erasure and rows it was authorised to remove in a cycle no delete order satisfies and is therefore a reference an authorised erasure has to be able to cut.
10. WHEN one transaction writes a spawning Event together with the child Session that Event spawns, THE Memory_Store SHALL insert the spawning Event before the child Session, because the statement inserting the Session requires the named Ledger row to be findable by that statement and writes nothing when it is not.
...
4. THE Provisioner SHALL create per Auditor one ccloud service account in the control plane together with one database login carrying a validity bound of at most 30 days computed when that login is created.
...
6. THE Provisioner SHALL make every Auditor query retrievable as a record of the CockroachDB_Cluster's own audit log through the audit-log pull of Requirement 27 criterion 8 rather than as a record the Telemetry emits, because an Auditor connects to the cluster endpoint directly and no Molt component stands in that request path to observe an Auditor's statement or the rows that statement returned, and THE retrieved record SHALL name the service account that ran the statement together with the statement or a digest of the statement, while THE control plane SHALL carry the count of rows that statement returned on that record for the row-count half of this criterion to hold, because the pull retrieves the records the control plane emits for a caller-supplied window and projects no field of its own.
...
11. THE Auditor_Gateway SHALL expose the as-of-attribution read of criterion 4 as a named member of the read-only view set of Requirement 24 criterion 5, that member projecting the Attribution_Version identifier, the Artifact identifier, the Client identifier, the detection method, the confidence value, the validity start timestamp, the validity end timestamp, and the superseding Attribution_Version identifier, so that the caller-supplied timestamp of criterion 4 is bound by the Auditor as a half-open containment predicate over the validity start timestamp and the validity end timestamp of that projection, because a view accepts no parameter and the instant is therefore the caller's to supply rather than the view's to hold.
