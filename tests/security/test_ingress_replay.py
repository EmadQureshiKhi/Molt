...
to the configured maximum request age (Requirement 47.14). This module is the
demonstration that the window is closed. That is a different claim from the one
the unit suite makes: there, the bound is shown to be computed correctly at the
microsecond in both directions; here, one request is captured as it went over
the wire, presented once and accepted, then presented again byte for byte after
the window has passed and refused.
...
def capture(
    ...
    *,
    ...
    presents. Such a request is refused now and becomes acceptable once the
    reading catches up, which is why the bound is an absolute difference.
    """
    moment = clock.now()
...
def test_a_request_stamped_ahead_of_the_reading_becomes_replayable_and_then_stops(
    time_source: ManualClock,
    recorded: Recorded,
) -> None:
    """A capture stamped into the future is a window that opens and then shuts.

    The request is stamped twice the bound ahead of the reading, so it is refused
    when it is made: nothing has shown it lies inside the window. As the reading
    catches up the difference falls to the bound and the identical bytes are
    accepted, and as the reading passes it the difference grows again and they are
    refused. That is why the bound is an absolute difference rather than a floor:
    a caller running fast is presenting bytes that will be replayable later, and
    the window they are replayable in is the same width as anyone else's.
    """
    factory = RefusedConnections()
    collector = build_collector(factory, time_source)
    captured = capture(time_source, stamped_ahead_by=timedelta(seconds=SCAN_SPAN_SECONDS))

    # The difference is twice the bound, so the request is ahead of its window.
    assert accepted_by(collector, captured, factory) is False

    # The reading reaches the near edge: the difference is exactly the bound.
    time_source.advance(MAX_AGE_SECONDS)
    assert accepted_by(collector, captured, factory) is True
    reached = factory.attempts

    # The reading passes the far edge by one step: the difference exceeds it.
    time_source.advance(SCAN_SPAN_SECONDS + ONE_MICROSECOND)
    assert accepted_by(collector, captured, factory) is False
    assert factory.attempts == reached
    assert recorded.causes() == (
        str(RejectionCause.OUTSIDE_WINDOW),
        str(RejectionCause.OUTSIDE_WINDOW),
    )
...
def test_the_window_a_capture_stays_replayable_in_is_the_configured_width(
    ...
    anticipated. The far side of the interval is the same width by the absolute
    difference, which the future-stamped sequence above exhibits directly.
